...
Example Usage:
    ...
    ;  msgbox dumpProps(hotkeySplit("^+!a up"))
    ;  msgbox dumpProps(hotkeySplit("~a & b up"))
...
hotkeyEqual(hotkeyA, hotkeyB, ignoreTildeAndDollarSign:=true)    {
    ...
    if (outA.Type=="Unknown" || outB.Type=="Unknown")
        return false
    for kA,vA in outA.ownProps()    {
        vB:=(outB.hasProp(kA)?outB.%kA%:"")
        if (kA~="D)Symbols$")    {
            vA0:=vA, vA:=""
            vB0:=vB, vB:=""
        ...
        {
            case (kA~="D)Key$"):
                if (regExMatch(vA, "iD)^(vk|sc)([[:xdigit:]]+)$", &mA))    {
                    modeA := format("{:L}", mA[1])
                    vA := format("{:L}", mA[1]) . format("{:0" (modeA == "sc" ? 3 : 2) "X}", "0x" mA[2])
                    if (regExMatch(vB, "iD)^" modeA "\K[[:xdigit:]]+$", &mB))    {
...
hotkeyGetValidation(hotkeyName)    {
    ...
    if (hotkeyName == "")    {
        ...
        hotIf(hotkeyValidationStub_B5962F5B)
        try  hotkey(hotkeyName, hotkeyValidationStub_B5962F5B, "Off")
        catch  {
            b := false 
        }  else  {
            b := true
...
hotkeyValidationStub_B5962F5B(*) => false
/*
...
formatKeyTitleCase(key)    {
    static list:=getListOfKeys_D27DF2D4()
    key:=format("{:L}",key)
    switch
    {
        default:                        return key
        case (list.has(key)):           return list[key]
        case regExMatch(key,"iD)^(sc|vk)([[:xdigit:]]+)$",&m):
            return (codeType:=format("{:U}",m[1])) format(codeType=="SC"?"{:03X}":"{:02X}","0x" m[2])
    }
}
getListOfKeys_D27DF2D4()    { ;  https://www.autohotkey.com/docs/v2/KeyList.htm
    static list
    if (!isSet(list))    {
        list:=map("lbutton","LButton"
        ,"rbutton","RButton"
        ,"mbutton","MButton"
        ,"xbutton1","XButton1"
        ,"xbutton2","XButton2"
